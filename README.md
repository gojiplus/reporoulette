...
- **IDSampler** — ≈ uniform over all existing public repositories with ID ≤ `max_id`. The closest thing to a true random sample of GitHub; the cost is a moderate hit rate (~37% measured live in 2026; the rest are deleted, private, or unassigned IDs). The default `max_id` is a dated constant — call `update_max_id()` (one API call) to cover the newest repositories.
